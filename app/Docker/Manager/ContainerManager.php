...
namespace App\Docker\Manager;

use App\Docker\Container;
use App\Docker\Exception\ContainerNotFoundException;
use App\Docker\Exception\UnexpectedStatusCodeException;
use App\Docker\Http\Stream\InteractiveStream;
use App\Docker\Json;
...
class ContainerManager
{
     ...
     * @param string $id
     *
     * @return \App\Docker\Container|null
     */
     ...
     * Inspect a container
     *
     * @param \App\Docker\Container $container
     *
    ...
    public function execstart($execid, callable $callback = null, $detach = false, $tty = false)
    {
        ...
        return new Stream((string) $response->getBody());
    }

    public function execInspect($execid)
    {
        $response = $this->client->get(['/exec/{id}/json', ['id' => $execid]], [
            'headers'      => ['content-type' => 'application/json'],
        ]);
     ...
     *
     * @param \App\Docker\Container $container Container to attach
     *
     ...
     *
     * @param \App\Docker\Container[]|array $containers
     * @param boolean                   $volumes
     *
     * @throws \App\Docker\Exception\UnexpectedStatusCodeException
     *
